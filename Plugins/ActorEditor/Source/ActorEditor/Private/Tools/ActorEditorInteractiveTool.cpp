...
#include "ActorEditorInteractiveTool.h"
...
#define LOCTEXT_NAMESPACE "UActorEditorInteractiveTool"

/*
 * ToolBuilder
 */

UInteractiveTool* UActorEditorInteractiveToolBuilder::BuildTool(const FToolBuilderState & SceneState) const
{
	UActorEditorInteractiveTool* NewTool = NewObject<UActorEditorInteractiveTool>(SceneState.ToolManager);
...
UActorEditorInteractiveToolProperties::UActorEditorInteractiveToolProperties()
{
...
void UActorEditorInteractiveTool::SetWorld(UWorld* World)
{
	check(World);
...
void UActorEditorInteractiveTool::Setup()
{
	...
	Properties = NewObject<UActorEditorInteractiveToolProperties>(this, "Measurement");
...
void UActorEditorInteractiveTool::OnUpdateModifierState(int ModifierID, bool bIsOn)
{
...
FInputRayHit UActorEditorInteractiveTool::CanBeginClickDragSequence(const FInputDeviceRay& PressPos)
{
...
void UActorEditorInteractiveTool::OnClickPress(const FInputDeviceRay& PressPos)
{
...
void UActorEditorInteractiveTool::OnClickDrag(const FInputDeviceRay& DragPos)
{
	UpdatePosition(DragPos.WorldRay);
}


FInputRayHit UActorEditorInteractiveTool::FindRayHit(const FRay& WorldRay, FVector& HitPos)
{
...
void UActorEditorInteractiveTool::UpdatePosition(const FRay& WorldRay)
{
...
void UActorEditorInteractiveTool::UpdateDistance()
{
	Properties->Distance = FVector::Distance(Properties->StartPoint, Properties->EndPoint);
}


void UActorEditorInteractiveTool::OnPropertyModified(UObject* PropertySet, FProperty* Property)
{
...
void UActorEditorInteractiveTool::Render(IToolsContextRenderAPI* RenderAPI)
{
